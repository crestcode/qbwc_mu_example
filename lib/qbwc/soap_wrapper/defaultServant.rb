...
class QBWC::QBWebConnectorSvcSoap
  ...
  def authenticate(parameters)
    #p parameters
    QBWC::AuthenticateResponse.new([parameters.strUserName, QBWC.company_file_path]) #path to company file
  end
  ...
  def sendRequestXML(parameters)
    username = parameters.ticket
    qbwc_session = QBWC::Session.new_or_unfinished(username)
  ...
  def closeConnection(parameters)
    #p [parameters]
    username = parameters.ticket
    qbwc_session = QBWC::Session.session(username)
